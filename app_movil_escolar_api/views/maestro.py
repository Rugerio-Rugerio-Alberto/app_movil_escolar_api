...
class MaestroView(generics.CreateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        try:
            ...
            maestros = get_object_or_404(Maestros, id=request.data.get("id"))
            
            # Actualizar campos con validación
            if "id_trabajador" in request.data:
                maestros.id_trabajador = request.data["id_trabajador"]
            if "fecha_nacimiento" in request.data:
                maestros.fecha_nacimiento = request.data["fecha_nacimiento"]
            if "telefono" in request.data:
                maestros.telefono = request.data["telefono"]
            if "rfc" in request.data:
                maestros.rfc = request.data["rfc"].upper()
            if "cubiculo" in request.data:
                maestros.cubiculo = request.data["cubiculo"]
            if "area_investigacion" in request.data:
                maestros.area_investigacion = request.data["area_investigacion"]
            ...
            if "materias_json" in request.data:
                ...
                if isinstance(materias, list):
                    maestros.materias_json = json.dumps(materias)
                else:
                    maestros.materias_json = materias
            ...
            return Response({"message": "Maestro actualizado correctamente", "maestro": MaestroSerializer(maestros).data}, 200)
        
        except Exception as e:
            print(f"Error en PUT maestro: {str(e)}")
